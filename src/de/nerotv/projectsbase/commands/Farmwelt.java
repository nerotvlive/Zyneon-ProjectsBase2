...
public class Farmwelt implements CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender s, final Command cmd, final String label, final String[] args) {
        if (cmd.getName().equalsIgnoreCase("Farmwelt")) {
            if (!(s instanceof Player)) {
                ...
                if(args.length == 0) {
                    ...
                    final World w = Bukkit.getWorld(this.loc.getString("Farmwelt.world"));
                    final double x = this.loc.getDouble("Farmwelt.X");
                    final double y = this.loc.getDouble("Farmwelt.Y");
                    final double z = this.loc.getDouble("Farmwelt.Z");
                    final float yaw = (float)this.loc.getDouble("Farmwelt.Yaw");
                    final float pitch = (float)this.loc.getDouble("Farmwelt.Pitch");
                    final Location loc2 = new Location(w, x, y, z, yaw, pitch);
                    p.teleport(loc2);
                    p.sendMessage(Main.Pro02 + " §8| §7Du bist nun in der §eFarmwelt§7!");
